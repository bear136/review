...
1. 通过正则解析html标签，将html标签转换成抽象语法树AST
  
  a. 首先会将html字符串根据正则进行截取，将截取到的每一段小字符串进行解析
  
  b. 维持一个栈，遇到开始标签就压入栈内，并保持栈中的最后一项是正在被解析的父节点，如果遇到结束标签则将该节点弹出
  
  c. 开始标签分为：文本标签、注释标签、doctype标签、条件注释等标签
  
2. 优化器优化节点，对静态节点打上标记

   a. 每次重新渲染的时候不需要为静态节点创造新节点

   b. 在虚拟dom中的patching过程会被跳过

3. 将抽象语法树转换成render函数字符串

   通过递归去拼一个函数执行代码的字符串，递归的过程根据不同的节点类型调用不同的生成方法，如果发现是一颗元素节点就拼一个 `_c(tagName, data, children)` 的函数调用字符串，然后 `data` 和 `children` 也是使用 `AST` 中的属性去拼字符串。
...
1. 对比当前同层的虚拟节点是否为同一种类型的标签，通过**sameVnode**方法进行判断，如果相同则通过**patchVnode进行深层次比较**，不同则**整个节点替换成新的虚拟节点**
